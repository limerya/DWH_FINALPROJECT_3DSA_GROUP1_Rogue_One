...
-- INCREMENTAL CHECK: Match against existing Fact rows
LEFT JOIN Fact_Order_Line_Items AS existing 
    ON dord.Order_Key = existing.Order_Key 
    AND dp.Product_Key = existing.Product_Key
-- FILTER: Only keep line items not already in Fact Table
